...
export function register() {
  suite("Flanking", () => {
    ...
    case_("_applyFlanked sets flankedBy flag and creates the Vulnerable AE", async (ctx) => {
      ...
      // v14: ActiveEffect.origin is a DocumentUUIDField, so non-UUID origin
      // strings get nulled at creation — use the module flag, not origin.
      const ae = actor.effects.find(e => e.getFlag(MODULE_ID, "flanking") === true);
    ...
    case_("_applyFlanked is idempotent — calling twice doesn't double-create AEs", async (ctx) => {
      ...
      const matching = actor.effects.filter(e => e.getFlag(MODULE_ID, "flanking") === true);
