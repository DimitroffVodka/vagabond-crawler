...
- **Gather Friendlies — per-hero right-click on the token HUD.** New button (paw icon) next to Foundry's default token controls on any character token. Teleports friendly NPC tokens on the scene that share a non-GM owner with the hero to free adjacent squares around the hero. Ownership filter prevents one player's summons from being pulled by another hero's gather. Already-near friendlies (within 2 squares) stay put. Uses 8-then-16 adjacency offsets; falls back to stacking on the hero if all 24 slots are occupied. Lives in new `scripts/gather-friendlies.mjs`.

### Combat Tracker Sync — Follow-ups
