...
- **Loot Generator relics auto-forge.** When the Loot Generator rolls a relic from a Level 2+ table, it matches the power text against `RELIC_POWERS` via the alias table and writes the same data the Forge would (effects, equip-gating, relic flag, added properties, on-hit statuses), without a Forge round-trip. This means most treasure items are functional out of the box.
- **Status powers use the system's own rules.** Bravery, Clarity and Repulsing add the status to the wearer's status resistances (Favor on the save). Burning I–III puts Burning (Cd4/d6/d8, fire tick damage) in the weapon's on-hit statuses. The cursed Anger, Cowardice and Gullibility skip the save, so the status lands.
- **Defensive and token powers work at the table.** Resistance halves damage of its type before Armor (Physical covers Blunt/Piercing/Slashing). Protection gives Favor on saves against attacks from its Being type (general matches the type, specific/niche match the creature's name or type). Cursed Doom caps a heal from a chat card at 1 per die. Nightvision gives the token darkvision; Truesight, Tremors, Echolocation and Sense Life add Foundry detection modes (Near range); Darkness, Moonlit and Radiant make the token shed darkness or light out to Close/Near/Far (Far is 60 ft). Unequipping removes them, and a torch lit over a relic light hands the relic light back when it's doused.
- **Activated powers live on the item's right-click menu.** Blast rolls 6d6 against every token in Close range with the usual save buttons. Precision makes the next attack with the relic hit. After-Image drops a translucent, unlinked copy of the wielder's token beside them (I: 1 round, II: 10 rounds; 6 s of world time per round outside combat). Invoke the Wish posts to chat for the GM and is gone for good. Benediction needs no click: while equipped, the first hit each week that would drop the wearer to 0 HP leaves them at 1. Store Spell: **Store a casting** picks dice/delivery/Effect and lowers the owner's Max Mana by its cost; **Cast stored …** casts it for no Mana and gives the Max Mana back.
- **Uses reset** on a Crawler **Rest** (per-day powers) or after 24 hours / 7 days of world time; Wish-Granting never resets. GMs get **Reset relic uses** on the menu.
- **Still up to the GM:** movement powers (Flying, Climbing, Jumping…), Telepathy, Detection, Sense Valuables, Ambassador, Aqua-Lung and Warning.
...
Sheet-level patches that make the Vagabond inventory behave the way Vagabond tables actually want it to. New items with a `quantity` field **auto-stack** with matching items (same name, same type) on creation, merging `quantity` and `baseCost` instead of producing duplicates — unless called with `skipStack: true` (lit torches, picked-up items, scrolls). **Quantity badges** render on items with `quantity > 1` in the inventory list. A stack of N costs `baseSlots × N` — two torches cost two slots. Vagabond 5.38+ counts this itself (and pools zero-slot items RAW: every 10 take one slot), so Crawler defers to the system's number. On older systems, which charge each item once and never read `quantity`, Crawler adds `baseSlots × (quantity − 1)` on top and treats **zero-slot items as free** (a Backpack, rations, coins, Unarmed). Either way, items **stowed inside a container** cost nothing because the container's own slots represent them, and the item-type filter mirrors the system's inventory set. Note that slot cost comes from the item's own `baseSlots`, not its name — the core `Candle` is zero-slot and therefore free, while the alchemical variants (`Candle, Basic` and friends) cost one each. Items flagged `trueZeroSlot` ("Weightless") cost their slot value once at any quantity and stay out of the 10-per-slot zero-slot pool. Alchemy Materials converted by VCE (quantity = silver value) cost 1 slot per 1g or part of one — 45s is 1 slot, 2g 50s is 3. Ammunition, torches, and alchemical items all behave sensibly.
