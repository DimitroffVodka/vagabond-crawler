...
System compendium light items (vagabond 5.23+) light through the system's **Use** instead of the right-click entry. The Crawler still makes lanterns and lamps need and burn oil, keeps a burned-out lantern, ticks hour clocks on crawl turns, and plays the light FX. Canvas drop, party transfer and the tracker window cover only the Crawler's own lights.
